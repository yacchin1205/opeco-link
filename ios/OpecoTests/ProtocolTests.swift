...
final class ProtocolTests: XCTestCase {
    ...
    func testFailedJoinRemainsReportedAfterAnotherOperationSucceeds() async {
        ...
        let staged = await model.join(link: "https://opeco.link/device#v=3&r=request_identifier&a=\(secret)&h=\(hash)")
    ...
    func testPairingLinkRequiresExactV3OrV4Fragment() throws {
        ...
        let link = try PairingLink("https://opeco.link/join#v=3&s=session_identifier&p=pairing_identifier&t=\(token)&a=\(secret)&k=\(publicKey)&c=ffd6e0")
        ...
        XCTAssertThrowsError(try PairingLink("https://opeco.link/join#v=2&s=session_identifier&p=pairing_identifier&t=\(token)&a=\(secret)&k=\(publicKey)&c=ffd6e0"))
        let invalidCurvePoint = Base64URL.encode(Data(repeating: 0, count: 65))
        XCTAssertThrowsError(try PairingLink("https://opeco.link/join#v=4&s=session_identifier&p=pairing_identifier&t=\(token)&a=\(secret)&k=\(invalidCurvePoint)&c=ffd6e0"))
    }
    ...
    func testDeviceRequestLinkAuthenticatesTheRequestAndApproval() throws {
        ...
        let link = try DeviceRequestLink("https://opeco.link/device#v=3&r=request_identifier&a=\(secret)&h=\(requestHash)")
        ...
        XCTAssertThrowsError(try DeviceRequestLink("https://example.com/device#v=3&r=request_identifier&a=\(secret)&h=\(requestHash)"))
        XCTAssertThrowsError(try DeviceRequestLink("https://opeco.link/device#v=3&r=request_identifier&a=\(secret)&h=\(requestHash)&g=group"))
    }
    ...
    func testTimestampSessionKeyMatchesECDHPeer() throws {
        ...
        let peer = peerSecret.hkdfDerivedSymmetricKey(
            ...
            sharedInfo: Data("opeco.link/session/v3\nsession-id\ngroup-id\n1789999000001".utf8), outputByteCount: 32
        )
    ...
    func testV4AttachmentUsesASeparateContextBoundKey() throws {
        ...
        let key = secret.hkdfDerivedSymmetricKey(
            using: SHA256.self, salt: Data(),
            sharedInfo: Data("opeco.link/attachment/v4\nsession\ngroup\n42\nresponse\nattachment".utf8), outputByteCount: 32
        )
        ...
        XCTAssertEqual(
            try AES.GCM.open(
                box, using: key,
                authenticating: Data("opeco.link/v4/attachment/session/group/42/response/attachment".utf8)
            ),
            jpeg
        )
    ...
    func testDismissResponseContainsOnlyDismissFields() throws {
        ...
        let plaintext = try AES.GCM.open(
            ...
            authenticating: Data("opeco.link/v3/response/session/group/42/response".utf8)
        )
    ...
    func testDeviceRequestQRCodeGeneration() {
        XCTAssertNotNil(InvitationQRCode.image(for: "https://opeco.link/device#v=2&r=request"))
